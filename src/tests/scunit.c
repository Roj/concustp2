 ...
 */
static void _get_suites_arr(size_t *num_suites, size_t *num_tests, scunit_suite_t **test_suites,
                            scunit_test_node_t *tests_list) {
...
static int _run_all_tests(void) {
  ...
  for (size_t i = 0; i < num_suites; i++) {
    ...
    /* prints suite header */
    printf("%s[----------]%s %zu test(s) from %s\n", _ok_ansi_seq, _normal_ansi_seq, suites[i].num_tests,
           suites[i].suite_name);
    ...
    for (size_t j = 0; j < suites[i].num_tests; j++) {
      ...
      printf("%s[ RUN      ]%s %s.%s\n", _ok_ansi_seq, _normal_ansi_seq, suites[i].suite_name,
      ...
      if (test_error != 0) {
        ...
        printf("%s[  FAILED  ]%s %s.%s (%llu ms)\n", _err_ansi_seq, _normal_ansi_seq, suites[i].suite_name,
               suites[i].tests[j]->test_name, (_get_timestamp( ) - test_start) / 1000000ull);
    ...
    printf("%s[----------]%s %zu test(s) from %s (%llu ms total)\n", _ok_ansi_seq, _normal_ansi_seq,
           suites[i].num_tests, suites[i].suite_name, (_get_timestamp( ) - suite_start) / 1000000ull);
  }

  /* prints footer */
  printf("%s[==========]%s %zu test(s) from %zu suite(s) ran (%llu ms total).\n", _ok_ansi_seq,
         _normal_ansi_seq, num_tests, num_suites, (_get_timestamp( ) - all_tests_start) / 1000000ull);
